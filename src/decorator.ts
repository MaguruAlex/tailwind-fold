...
export class Decorator {
    ...
    // Simplified regex to capture classes after double whitespaces
    regEx = /(class|className)(?:=|:|:\s)(['"`])([^'"`]*\s{2,})([^'"`]+)(['"`])/g
    ...
    updateDecorations() {
        ...
        let match
        while ((match = this.regEx.exec(documentText))) {
            // match[1]: 'class' or 'className'
            // match[2]: opening quote
            // match[3]: prefix before classes (including whitespaces)
            // match[4]: classes to fold
            // match[5]: closing quote

            const fullText = match[0]
            const textToFold = match[4]
            const prefixText = match[3]

            const foldStartIndex = fullText.indexOf(textToFold)
            const foldStartPosition = this.activeEditor.document.positionAt(match.index + fullText.indexOf(textToFold))
            const foldEndPosition = this.activeEditor.document.positionAt(
                match.index + fullText.indexOf(textToFold) + textToFold.length
            )
            const range = new Range(foldStartPosition, foldEndPosition)
